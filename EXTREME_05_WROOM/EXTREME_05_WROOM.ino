...
uint8_t defaultCommands[KEYMAP_COUNT][KEYS_PER_MAP] = {
  ...
  { 0x4F, 0x4F, 0x50, 0x50, 0x1D, 0x15, 0x51, 0x2C, 0x52, 0x51, 0x00, 0x00, 0x10, 0x0A },  // Map2 TerraPirata ext=M wheel=G
  { 0x52, 0x52, 0x51, 0x51, 0x52, 0x07, 0x51, 0x1D, 0x24, 0x25, 0x00, 0x00, 0x17, 0x17 }   // Map3 MRB     ext=T wheel=T
};
...
class OtaCharacteristicCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* pCharacteristic) {
    ...
    // Notifica progresso ogni 4KB circa
...
void handle_switch() {
  ...
  if (up && (now - lastSwitchMove >= SWITCH_REPEAT_DELAY)) {
    executeCommandById(currentAssignments[8]);
    lastSwitchMove = now;
  }
  if (down && (now - lastSwitchMove >= SWITCH_REPEAT_DELAY)) {
    executeCommandById(currentAssignments[9]);
    lastSwitchMove = now;
  }
}
...
void checkKeymapChange() {
  if (otaBleActive) return;
  unsigned long now = millis();
  bool k1 = TRAX.isPressed('1');
  bool k2 = TRAX.isPressed('2');
...
void setup() {
  Serial.begin(115200);
  Serial.println("=== HUGEROCK EXTREME PAD v03 WROOM ===");
  ...
  pinMode(SWITCH_UP, INPUT_PULLDOWN);
  pinMode(SWITCH_DOWN, INPUT_PULLDOWN);
